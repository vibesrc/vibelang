...
fn map<T, U>(arr: &Slice<T>, f: fn(&T) -> U) -> Array<U> {
    let result = Array<U>()
    for item in arr {
        result.push(f(item))
    }
...
let matrix: Array<f64[4]> = Array<f64[4]>()
...
```vibelang
Array<f64>[4]                   // fixed-size array of 4 f64
Array<f64>[4][4]                // 4x4 matrix
Array<Array<i32>[3]>[2]         // 2 arrays, each containing 3 i32
```

## 4.3.7 Type Constraints (Future)

> **Note:** Type constraints/bounds are not in v0.1 but may be added later.
...
fn sort<T: Comparable>(arr: &Array<T>) {
...
```vibelang
fn filter<T>(arr: &Slice<T>, pred: fn(&T) -> bool) -> Array<T> {
    let result = Array<T>()
    for item in arr {
        if pred(item) {
