...
| Type | Size | Range | LLVM Type |
|------|------|-------|-----------|
| `char` | 1 byte | 0 to 127 (ASCII) | `i8` |

The `char` type represents a single ASCII character. It is equivalent to `u8` but provides semantic meaning for character data.
...
let backslash: char = '\\'
let quote: char = '\''
let hex: char = '\x41'      // 'A' (hex escape)
let null: char = '\0'       // null character
```
...
// Character in string interpolation
println("char: ${c}")       // prints: char: A
```
...
> **Note:** `char` only supports ASCII (0-127). For Unicode support, use `String` or byte sequences.

## 4.1.5 String Slice Type

| Type | Size | Description | LLVM Type |
|------|------|-------------|-----------|
| `str` | 16 bytes | UTF-8 string slice (ptr + len) | `{ ptr, i64 }` |

The `str` type is a **distinct primitive type** representing a borrowed view into UTF-8 encoded string data. String literals have type `str`.
...
// Direct literal interpolation
println("value: ${"test"}") // value: test
```

### str vs Slice<u8>

While `str` and `Slice<u8>` have the same LLVM representation (fat pointer: `{ptr, i64}`), they are semantically distinct types:

| Type | Semantics | Use Case |
|------|-----------|----------|
| `str` | UTF-8 text data | String literals, text processing |
| `Slice<u8>` | Arbitrary byte sequence | Binary data, raw bytes |

```vibelang
let text: str = "hello"           // UTF-8 string literal
let bytes: Slice<u8> = get_data() // arbitrary bytes

// Type displays correctly in errors and IDE hover:
// - str shows as "str"
// - Slice<u8> shows as "Slice<u8>"
```

> **Note:** The identical LLVM representation means there is no runtime overhead to the type distinction. The separation is purely for semantic clarity and type safety.

For heap-allocated, growable strings, use `String` from the standard library.
...
For safe code, use `Slice<T>`, `Vec<T>`, or references `&T`.

## 4.1.8 Fixed Array Type
...
> **Note:** Vibelang requires explicit initialization. There is no implicit default initialization unless using a stdlib constructor. The `str` and `Slice<T>` types have no default value as they are slice/reference types that must point to valid data.
