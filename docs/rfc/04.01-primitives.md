...
|------|------|-------------|-----------|
| `str` | 16 bytes | String slice (ptr + len) | `{ ptr, i64 }` |

The `str` type is an alias for `Slice<u8>`, representing a borrowed view into UTF-8 encoded string data. String literals have type `str`.
...
`str` is semantically equivalent to `Slice<u8>`:

```vibelang
let s1: str = "hello"
let s2: Slice<u8> = "hello"     // same underlying type
```
...
> **Note:** Vibelang requires explicit initialization. There is no implicit default initialization unless using a stdlib constructor. The `str` type has no default value as it is a slice/reference type.
